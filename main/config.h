...
#define LLM_DEFAULT_MODEL_GEMINI "gemini-2.5-pro"

#define LLM_API_KEY_MAX_LEN 511
#define LLM_API_KEY_BUF_SIZE (LLM_API_KEY_MAX_LEN + 1)
#define LLM_AUTH_HEADER_BUF_SIZE                                               \
  (sizeof("Bearer ") - 1 + LLM_API_KEY_MAX_LEN + 1)
...
#define LLM_MAX_RETRIES                                                        \
  3 // Max LLM attempts per round (including first attempt)
...
#define SYSTEM_PROMPT                                                          \
  ...
  "You can create and run custom tools, control GPIO pins, store persistent "  \
  "memories, and set schedules. "                                              \
  "You run on the device itself, not as a separate cloud session. "            \
  "Be concise - you're on a tiny chip. "                                       \
  "Return plain text only. Do not use markdown, code fences, bullet lists, "   \
  "backticks, "                                                                \
  "bold, italics, or headings. "                                               \
  "Use your tools to control hardware, remember things, and automate tasks. "  \
  "When summarizing capabilities, prioritize custom tools, schedules, "        \
  "memory, and GPIO before optional i2c_scan details. "                        \
  "When asked for all or multiple GPIO states, prefer one gpio_read_all call " \
  "instead of repeated gpio_read calls. "                                      \
  "If users explicitly ask to view or change persona/tone settings, use "      \
  "set_persona/get_persona/reset_persona tools. "                              \
  "Persona is a persistent device setting on this ESP32 and survives reboot "  \
  "until changed or reset. "                                                   \
  "Do not change persona based on ambiguous wording or casual chat. "          \
  "When asked what is currently saved/set on the device, use tools to verify " \
  "instead of guessing. "                                                      \
  "Users can create custom tools with create_tool. When you call a custom "    \
  "tool, "                                                                     \
  "you'll receive an action to execute - carry it out using your built-in "    \
  "tools."
...
// OpenRouter can require tighter heap headroom during TLS setup on small
// targets. Use a shorter Telegram long-poll window only for that backend to
// reduce overlap.
#define TELEGRAM_POLL_TIMEOUT_OPENROUTER 8
// Classic ESP32 can exhaust/fragment heap when Telegram long-poll TLS overlaps
// with outbound LLM HTTPS on the same device. Keep poll windows shorter on that
// target.
...
#define TELEGRAM_STALE_POLL_RESYNC_STREAK                                      \
  8 // Trigger auto-resync after this streak
#define TELEGRAM_STALE_POLL_RESYNC_COOLDOWN_MS                                 \
  60000 // Min gap between auto-resync attempts
#define START_COMMAND_COOLDOWN_MS                                              \
  30000 // Debounce repeated Telegram /start bursts
#define MESSAGE_REPLAY_COOLDOWN_MS                                             \
  20000 // Suppress repeated identical non-command bursts
...
#define BOOT_SUCCESS_DELAY_MS                                                  \
  30000 // Clear boot counter after this time connected
